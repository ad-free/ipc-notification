...
class ActiveTestCase(TestCase):
	def setUp(self):
		...
		if not API_ALERT_LOCAL:
			self.url = API_ALERT_LIST['active'].format(
					protocol=API_ALERT_PROTOCOL,
					host=API_ALERT_HOST
			)
		else:
			self.url = API_ALERT_LOCAL_LIST['active'].format(
	...
	def test_update(self):
		with open(os.path.join(settings.BASE_DIR, 'apps/apis/test/test_case/test_case_api_active.json'), 'r') as f:
